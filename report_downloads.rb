module ApplicationController::ReportDownloads
  ...
  def download_csv(view, filename)
   ...
   rows = view.table.data.collect do |record|
     view.col_order.map do |col|
       val = record.data[col]
       case val
       when Array
         val.join(", ")
       when Numeric
       #  if col == "allocated_disk_storage" && val.to_i > 0
          if %w[size allocated_disk_storage].include?(col) && val.to_i > 0
           gb = val.to_f / (1024**3)
           # Remove .0 if it's a whole number, otherwise keep decimals
           gb_str = (gb % 1 == 0) ? gb.to_i.to_s : gb.round(2).to_s
           "#{gb_str} GB"
         else
           # Prevent Excel scientific notation for other numbers
           val.to_s =~ /^\d+$/ ? "=\"#{val}\"" : val.to_s
         end
       else
         val
       end
