class ManageIQ::Providers::Openstack::CloudManager < ManageIQ::Providers::CloudManager
  ...
  def create_network_port_new(options)
  # Translate cloud_network_id to OpenStack network_id if provided
  if options[:cloud_network_id] && !options[:network_id]
    network = CloudNetwork.find_by(:id => options[:cloud_network_id])
    raise ArgumentError, "CloudNetwork not found with id: #{options[:cloud_network_id]}" unless network
    options[:network_id] = network.ems_ref
  end
  
  raise ArgumentError, "network_id required" unless options[:network_id]

  with_provider_connection(:service => "network") do |service|
    port_params = { :name => options[:name] }
    port_params[:network_id] = options[:network_id]
    port_params[:admin_state_up]  = options[:admin_state_up] if options.key?(:admin_state_up)
    port_params[:mac_address]     = options[:mac_address] if options[:mac_address]
    port_params[:device_id]       = options[:device_id] if options[:device_id]
    port_params[:device_owner]    = options[:device_owner] if options[:device_owner]
    port_params[:fixed_ips]       = options[:fixed_ips] if options[:fixed_ips]
    port_params[:security_groups] = options[:security_groups] if options[:security_groups]
    
    _log.info("Creating network port: #{options[:name]} with params: #{port_params.inspect}")
    
    raw_port = service.ports.create(port_params)
    
    _log.info("Created port with ID: #{raw_port.id}")
    
    # Refresh to sync the new port back to ManageIQ
    EmsRefresh.queue_refresh(self)
    
    raw_port
  end
rescue => err
  _log.error("create_network_port_new error: #{err.class.name}: #{err.message}")
  _log.error(err.backtrace.join("\n"))
  raise MiqException::MiqNetworkPortCreateError, "Failed to create network port: #{err.message}"
end
...
def create_network_port_queue(userid, options = {})
  ...
  queue_opts = {
    ...
    :method_name => 'create_network_port_new',
    :instance_id => id,
